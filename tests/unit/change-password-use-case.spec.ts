...
describe('ChangePasswordUseCase', () => {
    ...
    beforeEach(() => { hasher = new BcryptHasher(4); jest.clearAllMocks(); });
    ...
    it('rejects when current password is invalid', async () => {
        ...
        repo.findById.mockResolvedValue({
            id: 'u1', name: 'U', email: 'u@example.com', username: 'user2025',
            password: hash, role: UserRole.TEACHER
        } as User);

        await expect(usecase.execute('u1', {
            currentPassword: 'wrong', newPassword: 'new-123456'
    ...
    it('updates password successfully', async () => {
        ...
        const user: User = {
            id: 'u1', name: 'U', email: 'u@example.com', username: 'user2025',
            password: oldHash, role: UserRole.TEACHER
        };
        repo.findById.mockResolvedValue(user);
        repo.update.mockImplementation(async (_id, data) => ({ ...user, ...data }) as User);

        await expect(usecase.execute('u1', {
            currentPassword: 'old-123456', newPassword: 'new-123456'
